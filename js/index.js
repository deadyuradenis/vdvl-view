...
(function (window) {
  var mount = function mount() {
    ...
    document.addEventListener('click', function (e) {
      var target = e.target;
      if (!target.closest('[data-component="header-search"]') && !target.closest('[data-header-search]') && component.classList.contains('is-open')) {
        hideSearch();
      }
    });
    window.app.headerSearch = {
      show: showSearch,
      hide: hideSearch
    };
  };
  ...
  var mount = function mount() {
    ...
    if (document.querySelector('[data-component="header-nav"]')) {
      ...
      var check = function check() {
        ...
        var triggerWidth = dropdownList.querySelectorAll('li').length > 1 ? 60 : 0;
        if (component.offsetWidth - triggerWidth < sourceList.offsetWidth) {
          appendToMobile();
        } else if (dropdownLast && component.offsetWidth - triggerWidth >= Number(sourceList.offsetWidth + dropdownLast.offsetWidth + 40)) {
          appendToMain();
        }
      ...
      var appendToMobile = function appendToMobile() {
        links = component.querySelectorAll('li');
        dropdownList.append(links[links.length - 1]);
        check();
      };
      var appendToMain = function appendToMain() {
        var tempLinks = dropdownList.querySelectorAll('li');
        sourceList.append(tempLinks[tempLinks.length - 1]);
        check();
      };
      var toMobile = function toMobile() {
      ...
      var showDropdown = function showDropdown() {
        ...
        if (window.innerWidth < 768) {
          window.app.backdrop.show(document.querySelector('header'));
        }
      };
      var hideDropdown = function hideDropdown(backdrop) {
        trigger.classList.remove('is-active');
        dropdown.classList.remove('is-open');
        if (window.innerWidth < 768) {
          if (backdrop) window.app.backdrop.hide();
        }
      };
      trigger.addEventListener('click', function () {
      ...
      document.addEventListener('click', function (e) {
        var target = e.target;
        if (!target.closest('[data-component="header-nav"]') && !target.closest('[data-header-nav]') && dropdown.classList.contains('is-open')) {
          hideDropdown(true);
        }
      });
      var windowWidth = window.innerWidth;
...
(function (window) {
  ...
  var handleBackdropClick = function handleBackdropClick() {
    if (window.app.headerNav) window.app.headerNav.hide();
    if (window.app.headerSearch) window.app.headerSearch.hide();
    hideBackdrop();
  };
